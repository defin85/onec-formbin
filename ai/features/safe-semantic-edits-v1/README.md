...
- the current writable subset covers `semantic/form.meta.json.form_title`, current form-scope `semantic/events.json[].handler`, and current `semantic/commands.json[].name/title`;
...
- `semantic/strings.json[].value` is now writable only as an alias for those
  same fixture-backed roles, including current command `name/title` aliases,
  plus current explicit `control_name` bridge items;
- the user-facing `apply-semantic` CLI flow is now fixture-backed for both the
  original form-title path, the current command-name/title and event-handler
  paths,
...
- the user-facing `apply-semantic` CLI flow is now also fixture-backed for the
  ...
  and command name/title together;
